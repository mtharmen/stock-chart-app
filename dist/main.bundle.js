...
var StocksComponent = (function () {
    ...
    StocksComponent.prototype.handleClick = function (company) {
        if (this.stocks.length < 2) {
            alert('Must have at least one stock at all times');
        }
        else {
            var code = this.remove(company);
            this.io.removeStock(code);
        }
    };
    StocksComponent.prototype.remove = function (company) {
        var code = company.split('(')[1].replace(')', '').trim();
        this.stocks = this.stocks.filter(function (stock) { return stock.indexOf(code) === -1; });
        return code;
    };
...
var SocketIOService = (function () {
    function SocketIOService() {
        ...
        this.throttled$ = new __WEBPACK_IMPORTED_MODULE_1_rxjs_Subject__["Subject"]();
